...
namespace MelodiiApp.UserInterface.Controls
{
    ...
    public partial class GestioneazaMelodiiControl : UserControl
    {
        ...
        /// <summary>
        /// Eveniment declanșat după ce datele melodiilor (ștergere, modificare) au fost actualizate.
        /// </summary>
        public event EventHandler MelodieDataChanged;
        ...
        public GestioneazaMelodiiControl()
        {
            ...
            if (statusStrip != null) 
            {
                ...
                statusStrip.ForeColor = ThemeHelper.TextColorOnLightAccent;
        ...
        private void BtnModificaMelodie_Click(object sender, EventArgs e)
        {
            if (dgvMelodii.SelectedRows.Count > 0)
            {
                ...
                if (selectedRow.DataBoundItem is Melodie melodieSelectata)
                {
                    using (var editForm = new EditMelodieForm(melodieSelectata))
                    {
                        if (editForm.ShowDialog() == DialogResult.OK)
                        {
                            RefreshData(); // Corrected LoadMelodii() to RefreshData()
                        }
                    }
                }
                else
                {
                    MessageBox.Show("Nu s-a putut obține obiectul melodie din rândul selectat.", "Eroare", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
            }
            else
            {
                MessageBox.Show("Vă rugăm selectați o melodie pentru a o modifica.", "Nicio Selecție", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
        }
                ...
                if (confirmation == DialogResult.Yes)
                {
                    try
                    {
                        ...
                        if (success)
                        {
                            ...
                            RefreshData(); // Local refresh
                            MelodieDataChanged?.Invoke(this, EventArgs.Empty); // Notify MainForm
                        }
                        else
                        {
